 /***********************************************************************
 * Project 3 - Bug2
 * Ramiro Velazquez, Uthira Mohan, Tariq Kottai
 * 05/04/2016
 * 
 * 
 * 
 ...
 ***********************************************************************/
  ...
  //Provides special objects, methods, and f
  //functions for the RedBot.
 ...
 int rightSpeed;  // variable used to store the rightMotor speed
  ...
  float m= (y_g)/(x_g);//slope
  int i=1;//to determine the linear accleration
  void setup()
  {
...
float dh = sqrt(sq(x_g-xh) + sq(y_g-yh));
float m= (y_g)/(x_g);

float    s=abs( ((y_g)*x)-((x_g)*y));

float  d = sqrt(sq(x_g-x) + sq(y_g-y));
Serial.println("s,d,dh \t s \t");
Serial.print(s);
Serial.print("\t d");
Serial.println(d);
Serial.print("\t dh");
Serial.println(dh);
   //call follow path.
        ...
        Serial.println("check something..."); 
              if((s<10))                               //  to find points in mline    
              {
                Serial.println(s);
                Serial.println("close");
                 if((d<dh)&&(y<=y_g))           // to check if it is leave point          
                 {
                   Serial.println("leave point found");
                  LeaveLine =0;
                  obstacle++;
                  motors.stop();
                  delay(1200);
